...
export function resolveProfile(baseUrl: string | undefined, fallbackProfile = 'web'): ProfileInfo {
...
export type PnpmProgressFn = (p: { phase: 'resolving' | 'fetching' | 'linking' | 'done'; percent: number | null; detail: string }) => void;
...
export function parsePnpmProgress(line: string): { phase: 'fetching' | 'done'; percent: number | null } | null {
  ...
  if (/^Packages: \+/i.test(line)) return { phase: 'fetching', percent: null };
  return null;
}
...
export function runPnpm(cwd: string, args: string[], timeoutMs = 5 * 60_000, onProgress?: PnpmProgressFn): Promise<PnpmResult> {
  return new Promise((resolve) => {
    ...
    const feed = (chunk: string) => {
      ...
      for (const line of lines) {
        ...
        if (parsed) onProgress({ phase: parsed.phase, percent: parsed.percent, detail: line.slice(0, 80) });
      }
    };
    child.stdout?.on('data', (d: Buffer) => { const s = d.toString(); stdout += s; feed(s); });
...
function readProfileManifest(filename: string): Record<string, unknown> {
  if (!existsSync(filename)) return { dependencies: {}, dsh: { profile: { bundles: [] } } };
  try {
...
function exportsBundlePatch(packageName: string, profileDir: string): boolean {
  try {
...
export function bundleEntryIds(packageName: string, profileDir: string): string[] {
  const ids: string[] = [];
  try {
    const require = createRequire(join(profileDir, 'noop.js'));
    const manifestPath = require.resolve(`${packageName}/package.json`);
    ...
    const dsh = manifest.dsh as Record<string, unknown> | undefined;
    const bundle = dsh?.bundle as Record<string, unknown> | undefined;
    if (typeof bundle?.patch !== 'string') return ids;
    const patchFile = join(dirname(manifestPath), (bundle.patch as string).replace(/^\.\//, ''));
    if (!existsSync(patchFile)) return ids;
    const document = parseDocument(readFileSync(patchFile, 'utf8'));
    if (!isSeq(document.contents)) return ids;
    for (const item of document.contents.items) {
      ...
      const rows: Array<Record<string, unknown>> = insert === null || insert === undefined
        ? []
        : isSeq(insert)
          ? insert.toJSON() as Array<Record<string, unknown>>
...
function reconcileBundles(profile: ProfileInfo, added: string[], removed: string[]): void {
  const manifest = readProfileManifest(profile.packageJsonFile);
  const deps = (manifest.dependencies ?? {}) as Record<string, string>;
  const dsh = (manifest.dsh ?? {}) as Record<string, unknown>;
  const profileObj = (dsh.profile ?? {}) as Record<string, unknown>;
  ...
  for (const name of removed) {
    ...
    if (at >= 0) { bundles.splice(at, 1); changed = true; }
  }
  if (!changed) return;
  manifest.dsh = {
    ...dsh,
...
async function readPatchDocument(filename: string): Promise<Document> {
  let source: string;
  try {
...
function isOwned(map: unknown): boolean {
  return isMap(map) && map.commentBefore?.includes('Managed by dsh-plugin-hub') === true;
}

function ownedInsertRow(document: Document, packageName: string): YAMLMap | undefined {
  const seq = document.contents as YAMLSeq;
  return seq.items.find((item) => {
    if (!isMap(item) || !isOwned(item)) return false;
    const insert = item.get('insert');
    if (!Array.isArray(insert)) return false;
    return insert.some((e: unknown) => isMap(e) && e.get('id') === packageName);
  }) as YAMLMap | undefined;
}

/** Find an owned insert row targeting a specific loader entry id. */
function ownedInsertRowById(document: Document, entryId: string): YAMLMap | undefined {
  const seq = document.contents as YAMLSeq;
  return seq.items.find((item) => {
    if (!isMap(item) || !isOwned(item)) return false;
    const insert = item.get('insert');
    if (!Array.isArray(insert)) return false;
    return insert.some((e: unknown) => isMap(e) && e.get('id') === entryId);
  }) as YAMLMap | undefined;
}

function ownedToggleRow(document: Document, packageName: string): YAMLMap | undefined {
  const seq = document.contents as YAMLSeq;
  return seq.items.find((item) => {
    if (!isMap(item) || !isOwned(item)) return false;
...
async function removeOwnedRows(filename: string, packageName: string): Promise<void> {
  const document = await readPatchDocument(filename);
  const seq = document.contents as YAMLSeq;
  const kept = seq.items.filter((item) => {
    if (!isMap(item) || !isOwned(item)) return true;
    if (item.get('id') === packageName) return false;
    const insert = item.get('insert');
    if (Array.isArray(insert) && insert.some((e: unknown) => isMap(e) && e.get('id') === packageName)) return false;
    return true;
  });
  if (kept.length === seq.items.length) return;
...
export async function addActivationRow(profile: ProfileInfo, packageName: string, entryId = packageName): Promise<void> {
  if (isInBundles(profile, packageName)) return;
  ...
  if (existing) {
    const insert = existing.get('insert') as unknown[];
    if (Array.isArray(insert)) {
      for (const e of insert) {
        if (isMap(e) && e.get('id') === entryId) { e.set('name', packageName); }
      }
    }
    await atomicWrite(profile.patchFile, String(document));
    return;
  }
  const row = document.createNode({ insert: [{ id: entryId, name: packageName }] }) as YAMLMap;
...
function isInBundles(profile: ProfileInfo, packageName: string): boolean {
  try {
    const manifest = readProfileManifest(profile.packageJsonFile);
    const dsh = (manifest.dsh ?? {}) as Record<string, unknown>;
    const profileObj = (dsh.profile ?? {}) as Record<string, unknown>;
    return Array.isArray(profileObj.bundles) && (profileObj.bundles as unknown[]).includes(packageName);
  } catch {
    return false;
  }
}
...
export async function writeEnablement(profile: ProfileInfo, entryId: string, moduleName: string, enabled: boolean): Promise<void> {
  const document = await readPatchDocument(profile.patchFile);
  const seq = document.contents as YAMLSeq;
  let row = ownedToggleRow(document, entryId);
  if (row === undefined) {
    row = document.createNode({ id: entryId, name: moduleName, disabled: !enabled }) as YAMLMap;
    row.commentBefore = OWNER_MARKER;
    seq.add(row);
  } else {
...
async function removeToggleRow(filename: string, entryId: string): Promise<void> {
  const document = await readPatchDocument(filename);
  const seq = document.contents as YAMLSeq;
...
export function installSpecFor(packageName: string, repoFullName: string, ref: string | null): string {
  return `${packageName}@${ref ?? 'latest'}`;
}

export function gitSpecFor(repoFullName: string, ref: string | null): string {
 ...
 */
export async function installPlugin(profile: ProfileInfo, installSpec: string, entryId: string, actor: string, onProgress?: PnpmProgressFn): Promise<InstallReceipt> {
  ...
  if (missingDirs.length > 0) {
    ...
    if (entryIds.length > 0) {
      ...
      for (const id of entryIds) await writeEnablement(profile, id, name, false);
    } else {
...
export function scanRequiredEnv(packageName: string, profileDir: string): string[] {
  const require = createRequire(join(profileDir, 'noop.js'));
  ...
  try {
    ...
    const walk = (d: string) => {
      ...
      for (const e of entries) {
        ...
        else if (st.size <= 200 * 1024 && /\.(?:js|mjs|cjs|ts|tsx|jsx)$/.test(e)) targets.push(full);
      }
    };
    walk(dir);
    ...
    for (const name of ['README.md', 'readme.md', 'README.MD']) {
      ...
      for (const m of text.matchAll(/^([A-Z][A-Z0-9_]{2,})\s*=\s*.{0,40}$/gm)) {
        if (!/^(?:NODE|PATH|HOME|SHELL|USER|TERM|LANG|EDITOR|TMP|CI)/.test(m[1]!)) found.add(m[1]!);
      }
    }
...
export function runGit(cwd: string, args: string[], timeoutMs = 3 * 60_000): Promise<PnpmResult> {
  return new Promise((resolve) => {
...
export async function rmrf(target: string): Promise<void> {
  try { await rm(target, { recursive: true, force: true }); } catch { /* ignore */ }
}

/** Uninstall a plugin package from the profile (removes entry + bundles). */export async function uninstallPlugin(profile: ProfileInfo, packageName: string, entryId: string, actor: string): Promise<InstallReceipt> {
