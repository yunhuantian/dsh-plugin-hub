...
function walkFiles(dir: string, base: string, out: string[]): void {
  ...
  for (const name of entries) {
    ...
    else if (st.isFile() && st.size <= MAX_FILE && /\.(?:js|mjs|cjs|ts|tsx|jsx)$/.test(name)) out.push(full);
  }
}

/** Extract slot names a plugin registers into (scan its client sources). */
export function scanRegisteredSlots(profile: ProfileInfo, packageName: string): Set<string> {
  const slots = new Set<string>();
  const require = createRequire(join(profile.directory, 'noop.js'));
  let pkgPath: string | null = null;
  try { pkgPath = require.resolve(`${packageName}/package.json`); } catch { return slots; }
  const dir = pkgPath.replace(/[\\/]package\.json$/, '');
  const files: string[] = [];
  walkFiles(dir, dir, files);
  // Also read the package.json `dsh.client` main if present.
  let pkg: Record<string, unknown> = {};
  try { pkg = JSON.parse(readFileSync(pkgPath, 'utf8')) as Record<string, unknown>; } catch { /* noop */ }
  const client = (pkg.dsh as Record<string, unknown> | undefined)?.client as Record<string, unknown> | undefined;
  if (client && typeof client.main === 'string') {
    try { files.push(join(dir, client.main)); } catch { /* noop */ }
  }
  for (const file of files.slice(0, 300)) {
    let text = '';
    try { text = readFileSync(file, 'utf8'); } catch { continue; }
    SLOT_RE.lastIndex = 0;
    for (let m = SLOT_RE.exec(text); m; m = SLOT_RE.exec(text)) slots.add(m[1]!);
  }
  return slots;
}

/** Read a plugin's dependencies (own manifest). */
function pluginDeps(profile: ProfileInfo, packageName: string): Record<string, string> {
  const require = createRequire(join(profile.directory, 'noop.js'));
  try {
...
function installedDepNames(profile: ProfileInfo, except: string): Set<string> {
  const set = new Set<string>();
  try {
    const manifest = JSON.parse(readFileSync(profile.packageJsonFile, 'utf8')) as Record<string, unknown>;
    const deps = (manifest.dependencies ?? {}) as Record<string, string>;
    for (const name of Object.keys(deps)) if (name !== except) set.add(name);
  } catch { /* unreadable */ }
  return set;
}

/** Check the new plugin's activation patch references resolvable packages. */
function checkPatchIntegrity(profile: ProfileInfo, packageName: string): string[] {
  const problems: string[] = [];
  const require = createRequire(join(profile.directory, 'noop.js'));
  let pkgPath: string | null = null;
  try { pkgPath = require.resolve(`${packageName}/package.json`); } catch { return problems; }
  let pkg: Record<string, unknown> = {};
  try { pkg = JSON.parse(readFileSync(pkgPath, 'utf8')) as Record<string, unknown>; } catch { return problems; }
  const bundle = (pkg.dsh as Record<string, unknown> | undefined)?.bundle as Record<string, unknown> | undefined;
  const patchRef = bundle?.patch;
  if (typeof patchRef !== 'string' || !patchRef) return problems;
  const dir = pkgPath.replace(/[\\/]package\.json$/, '');
  const patchFile = join(dir, patchRef);
  let patchText = '';
  try { patchText = readFileSync(patchFile, 'utf8'); } catch { return problems; }
  const installed = installedDepNames(profile, packageName);
  const insertNames = new Set<string>();
  for (const m of patchText.matchAll(/insert:\s*\[?[^\n]*|name:\s*([^\s,}]+)/g)) {
    if (m[1]) insertNames.add(m[1]);
  }
  for (const name of insertNames) {
    if (installed.has(name)) continue;
    // platform / loader entry ids (base bundles) are provided by the app.
    if (/^@deepseek-ai\//.test(name) || /^(api-gateway|api-remotes|connection|client-hmr|client-locale|client-modules|client-runtime|hmr|include|locale|modules|runtime|timer|webserver|cordis-host-runner|ui-settings)$/.test(name)) continue;
...
export function checkConflicts(profile: ProfileInfo, packageName: string): ConflictReport {
