...
function parseEntry(text: string): { name: string; trust: string; install: string; repo: string; publisher: string } | null {
  const e = { name: '', trust: '', install: '', repo: '', publisher: '' };
  let inSource = false;
  let inPublisher = false;
  for (const line of text.split('\n')) {
    const t = line.trim();
    if (t.startsWith('source:')) { inSource = true; inPublisher = false; continue; }
    if (t.startsWith('publisher:')) { inPublisher = true; inSource = false; continue; }
    if (line && !line.startsWith(' ') && !line.startsWith('#')) { inSource = false; inPublisher = false; }
    if (inSource) {
      if (t.startsWith('repo:')) {
        const v = t.slice(5).trim();
        if (v && v !== 'null') e.repo = extractRepo(v);
      }
      continue;
    }
    if (inPublisher) {
      if (t.startsWith('github:')) {
        ...
        if (v && v !== 'null') e.publisher = v;
      }
      continue;
    }
    if (t.startsWith('name:')) e.name = t.slice(5).trim();
    if (t.startsWith('trust:')) e.trust = t.slice(6).trim();
    if (t.startsWith('install:')) e.install = t.slice(8).trim();
  }
  return e.name || e.repo || e.install ? e : null;
}

export async function syncTrustLevels(github: GitHubClient, db: StoreDB): Promise<TrustSyncResult> {
  try {
    // 1) list the index directory.
    const files = await github.apiGet<Array<{ name?: string; type?: string }>>(
      `/repos/${INDEX_OWNER}/${INDEX_REPO}/contents/${INDEX_DIR}`,
    );
    const ymls = (files ?? [])
      .filter((f) => f.type === 'file' && (f.name?.endsWith('.yml') || f.name?.endsWith('.yaml')))
      .map((f) => f.name as string);
    if (ymls.length === 0) return { ok: false, updated: 0, matched: 0, error: 'index dir empty/unreachable' };

    // 2) fetch and parse each entry (bounded; individual failures skipped).
    const byKey = new Map<string, TrustEntry>(); // package_name / repo_full_name → trust
    for (const file of ymls.slice(0, 300)) {
      try {
        const text = await github.rawFile(INDEX_OWNER, INDEX_REPO, 'main', `${INDEX_DIR}/${file}`);
        if (!text) continue;
        const entry = parseEntry(text);
        if (!entry || !entry.trust) continue;
        const item: TrustEntry = { trust: entry.trust, publisher: entry.publisher || undefined };
        ...
        if (entry.repo) byKey.set(entry.repo.toLowerCase(), item);
