...
function resolveEntryFile(packageName, profileDir) {
    try {
...
function declaredDeps(packageName, profileDir) {
    const set = new Set();
    try {
        const require = createRequire(join(profileDir, 'noop.js'));
        const manifestPath = require.resolve(`${packageName}/package.json`);
...
function expandCandidatePath(raw, home) {
    ...
    if (value.startsWith(home) || /^[A-Za-z]:[\\/]/.test(value))
        return value;
    return null;
}
/**
 ...
 */
function extractDirectoryCandidates(source, home, variableBase) {
    const found = [];
    const seen = new Set();
    ...
    while ((m = homeJoin.exec(source)) !== null) {
        const parts = [m[2], m[4], m[6]].filter((p) => typeof p === 'string');
 ...
 */
export function runQualityGate(packageName, profileDir) {
    readCache.clear();
    ...
    while (queue.length > 0) {
        ...
        for (const spec of extractSpecifiers(source)) {
            if (isBuiltin(spec))
                continue;
            if (PLATFORM_PREFIXES.some((p) => spec === p || spec.startsWith(p)))
                continue;
            // Count every external (non-builtin, non-platform) spec we inspected.
            if (!scannedSpecs.has(spec))
            ...
            if (spec.startsWith('.') && depth < 2) {
                ...
                for (const tryPath of [candidate, `${candidate}.js`, join(candidate, 'index.js')]) {
                    if (!visited.has(tryPath)) {
                        ...
                        if (src !== null) {
                            queue.push({ file: tryPath, depth: depth + 1 });
                            break;
                        }
                    }
                }
