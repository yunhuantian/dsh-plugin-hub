...
export class StoreDB {
    ...
    constructor(home = resolveDshHome()) {
        ...
        try {
            this.db.exec("ALTER TABLE skills ADD COLUMN platform TEXT NOT NULL DEFAULT 'general'");
        }
        catch { /* column already exists */ }
    }
    close() {
    ...
    replaceSkills(skills) {
        const stmt = this.db.prepare('INSERT OR REPLACE INTO skills (id, name, description, description_zh, version, author, repo_full_name, skill_path, categories, platform, content, synced_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        const now = new Date().toISOString();
        let n = 0;
        for (const s of skills) {
            stmt.run(s.id, s.name, s.description, s.descriptionZh ?? '', s.version ?? '', s.author ?? '', s.repoFullName ?? '', s.skillPath ?? '', JSON.stringify(s.categories ?? []), s.platform ?? 'general', s.content ?? '', now);
            n++;
        }
        return n;
    }
    ...
    allSkills() {
        const rows = this.db.prepare('SELECT * FROM skills ORDER BY synced_at DESC').all();
        return rows.map((r) => ({
            id: String(r.id),
            name: String(r.name ?? ''),
            ...
            platform: (String(r.platform ?? 'general') === 'dsh' ? 'dsh' : 'general'),
